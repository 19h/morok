...
/* --- ZORYA addition (public domain, same TU so static helpers are visible) ---
 * Recompute the Ed25519 verification point t = pack(S*B - h*A) for a signed
 * message sm (sig||msg) under public key pk, WITHOUT branching on validity.
 * For a genuine signature t == sm[0:32] (==R). Callers fold t into a key, so a
 * forgery would require producing a signature that recomputes to the sealed t
 * (i.e. breaking Ed25519) rather than flipping a comparison. */
int z_ed25519_recompute(u8 tout[32], const u8 *sm, u64 n, const u8 *pk)
{
  int i;
  u8 h[64];
  static u8 mbuf[1024];
  gf p[4], q[4];
  if (n < 64 || n > sizeof mbuf) return -1;
  if (unpackneg(q, pk)) return -1;
  FOR(i, (int)n) mbuf[i] = sm[i];
  FOR(i, 32) mbuf[i + 32] = pk[i];
  crypto_hash(h, mbuf, n);
  reduce(h);
  scalarmult(p, q, h);
  scalarbase(q, sm + 32);
  add(p, q);
...
/* ZORYA addition: Ed25519 keypair (issuer-only); mirror commented theirs out. */
int z_ed25519_keypair(u8 *pk, u8 *sk)
{
  u8 d[64]; gf p[4]; int i;
  randombytes(sk, 32);
  crypto_hash(d, sk, 32);
