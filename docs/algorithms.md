...
- Eligibility is deliberately narrow (a wrong value must be a valid failure
  ...
  and no personality/`naked`/coroutine, is not in a direct-call cycle (bounded
  DFS), is within `max_instructions`, and has no observable side effects (any
...
- The hub is a single straight-line, branchless block.  A per-hub volatile epoch
  global is advanced by an odd step each call and reduced modulo `clone_count`
  to pick the real clone (per-invocation variation even on clean runs).  A
  zero-on-clean seal key is derived from the `anti_debug`, `env_binding`, and
  `tracer` RuntimeSeal channel deltas: an `anyDirty` OR guarantees the KDF seed
  is nonzero whenever any channel is dirty (so an XOR cancellation can never
  launder a dirty state to clean), and `emitKdf64` yields `0` iff clean.  The
  final index is `select dirty, clone_count + (key % counterfeit_count),
  epoch % clone_count` — a branchless selector, not a conditional branch — loaded
  from the table by an in-bounds GEP and called indirectly with `F`'s arguments.
  If a channel has no producer it stays clean and contributes zero, so enabling
...
- Attribute hygiene: the hub's epoch store and indirect call relax `F`'s memory-
  effect/`readonly`/`willreturn`/`norecurse`/`speculatable` promises (never its
  ABI return/parameter attributes); the indirect call keeps `nounwind` when `F`
  had it.  All candidates and the epoch/table/per-fake-table globals are
  private-linkage, so their `__mirage` names are assembler-local and never reach
