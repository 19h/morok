...
TEST_CASE("high preset matches the documented table") {
    ...
    CHECK(c.virtualization.enabled == false);
    CHECK(c.virtualization.probability == 0u);
    CHECK(c.virtualization.max_functions == 0u);
    CHECK(c.virtualization.max_instructions == 0u);
    CHECK(c.virtualization.max_registers == 0u);
    CHECK(c.hash_self_decrypt.enabled == false);
    CHECK(c.hash_self_decrypt.probability == 0u);
    CHECK(c.hash_self_decrypt.max_payloads == 0u);
    CHECK(c.hash_self_decrypt.context_keying == false);
    CHECK(c.self_checksum.enabled == false);
    CHECK(c.self_checksum.probability == 0u);
    CHECK(c.self_checksum.max_constants == 0u);
    CHECK(c.self_checksum.region_bytes == 0u);
    CHECK(c.data_flow_integrity.enabled == false);
    CHECK(c.data_flow_integrity.probability == 0u);
    CHECK(c.data_flow_integrity.max_tables == 0u);
    CHECK(c.data_flow_integrity.region_bytes == 0u);
    CHECK(c.mutual_guard.enabled == false);
    ...
    CHECK(c.shamir_share.enabled == true);
    CHECK(c.shamir_share.probability == 30u);
    CHECK(c.shamir_share.threshold == 3u);
    CHECK(c.shamir_share.shares == 5u);
    CHECK(c.shamir_share.max_secrets == 8u);
    CHECK(c.mq_gate.enabled == false);
    CHECK(c.mq_gate.probability == 0u);
    CHECK(c.mq_gate.vars == 0u);
    CHECK(c.mq_gate.eqs == 0u);
    CHECK(c.mq_gate.density == 0u);
    CHECK(c.mq_gate.max_gates == 0u);
    CHECK(c.mq_gate.fold_diff == false);
    CHECK(c.adversarial_merge.enabled == false);
    ...
    CHECK(c.dispatcherless.max_terms == 3u);
    CHECK(c.microcode_stress.enabled == false);
    CHECK(c.microcode_stress.probability == 0u);
    CHECK(c.microcode_stress.max_sites == 0u);
    CHECK(c.microcode_stress.table_entries == 0u);
    CHECK(c.microcode_stress.decoy_blocks == 0u);
    CHECK(c.microcode_stress.alias_stores == 0u);
    ...
    CHECK(c.csm.tf_const == 0u);
    CHECK(c.flatten.enabled == false);
    CHECK(c.func_wrap.enabled == false);
