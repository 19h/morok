...
TEST_CASE("merge handles every pass family") {
    ...
    src.mirage.enabled = true;
    src.mirage.sensitive_only = false;
    src.mirage.clone_count = 3u;
    src.mirage.counterfeit_count = 2u;
    src.mirage.max_functions = 12u;
    src.mirage.max_instructions = 400u;
    src.mirage.counterfeit_domains = {"license_check"};
    src.mirage.seal_gated_reality = true;
    src.mirage.per_invocation_epoch = false;
    src.mirage.cross_guard = true;
    src.mirage.force_route = "fake";
    ...
    CHECK(dst.mirage.enabled == true);
    CHECK(dst.mirage.sensitive_only == false);
    CHECK(dst.mirage.clone_count == 3u);
    CHECK(dst.mirage.counterfeit_count == 2u);
    CHECK(dst.mirage.max_functions == 12u);
    CHECK(dst.mirage.max_instructions == 400u);
    REQUIRE(dst.mirage.counterfeit_domains.size() == 1);
    CHECK(dst.mirage.counterfeit_domains[0] == "license_check");
    CHECK(dst.mirage.seal_gated_reality == true);
    CHECK(dst.mirage.per_invocation_epoch == false);
    CHECK(dst.mirage.cross_guard == true);
    CHECK(dst.mirage.force_route == "fake");
