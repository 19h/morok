    ...
    for (const BasicBlock &BB : F)
        for (const Instruction &I : BB)
            if (isa<InvokeInst>(I) || isa<CallBrInst>(I) ||
                isa<LandingPadInst>(I) || isa<CatchSwitchInst>(I) ||
...
void buildHub(Function &F, GlobalVariable *Table, unsigned CloneCount,
    ...
    const unsigned TableLen = CloneCount + FakeCount;
    auto *TableTy = ArrayType::get(PtrTy, TableLen);
    ...
    Value *Slot = B.CreateInBoundsGEP(TableTy, Table,
                                      ...
                                      "mirage.slot");
    auto *Target = B.CreateLoad(PtrTy, Slot, "mirage.target");
